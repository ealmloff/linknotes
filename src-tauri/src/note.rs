...
Programmer’s Name: Evan Almoff, Suhaan Syed
...
Tags (Vec<Tag>): Each tag must have a valid name and manual flag.
Text (String): Any non-empty string.
Workspace ID (WorkspaceId): Must reference a valid workspace.
Unacceptable Inputs:
...
struct ContextualDocumentLocation { // Define the `ContextualDocumentLocation` struct.
...
impl AsRef<Document> for ContextualDocument {
    fn as_ref(&self) -> &Document { // Implement the `as_ref` method for the `ContextualDocument` struct.
        &self.document // Return a reference to the document.
    } 
}

#[derive(Debug, Clone, Serialize, Deserialize, Eq, Ord)]
pub struct Tag { // Define the `Tag` struct.
    pub name: String, // A `String` field to store the tag name.
    pub manual: bool, // A `bool` field to indicate if the tag is manual.
}

impl PartialEq for Tag {
    fn eq(&self, other: &Self) -> bool { // Implement the `eq` method for the `Tag` struct.
        self.name == other.name && self.manual == other.manual // Compare the tag name and manual flag for equality.
    }
}

impl PartialOrd for Tag {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> { // Implement the `partial_cmp` method for the `Tag` struct.
...
#[derive(Serialize, Deserialize, PartialEq)] // Implement the `Serialize` and `Deserialize` traits for the `Segment` struct.
struct Segment {
...
pub async fn save_note(
    ...
    if current_location.is_none() { // If the document does not exist, create it.
...
pub async fn read_note( // Define the `read_note` function as a Tauri command.
    title: String, // Define the `title` parameter as a `String`.
    workspace_id: WorkspaceId, // Define the `workspace_id` parameter as a `WorkspaceId`.
) -> Result<ContextualDocument, DocumentDoesNotExistError> { // Define the return type of the function.
    let workspace = get_workspace_ref(workspace_id);
    let document_table = workspace.document_table().await.unwrap();
    let db = document_table.table().db(); // Get the database connection for the document table.
    let location: ContextualDocumentLocation = db // Retrieve the document location based on the title.
